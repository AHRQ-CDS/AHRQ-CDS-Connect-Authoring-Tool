...
const ElementCardHeaderActions = ({
  ...
  return (
    <>
      {showContent && (
        <Tooltip title={showComment ? 'Hide Comment' : 'Show Comment'} arrow>
          <span>
            <IconButton
      ...
      <Tooltip title={showContent ? 'Collapse' : 'Expand'} arrow>
        <span>
      ...
      <Tooltip title={disableDeleteMessage || 'Delete'} arrow>
        <span>
          <IconButton
            aria-label="delete"
